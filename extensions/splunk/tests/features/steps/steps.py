...
def step_impl(context: MinifiTestContext):
    ...
    assert context.containers["splunk"].deploy() or context.containers["splunk"].log_app_output()
    assert context.containers["splunk"].enable_splunk_hec_indexer('splunk_hec_token')


@then('an event is registered in Splunk HEC with the content \"{content}\"')
def step_imp(context, content):
    assert context.containers["splunk"].check_splunk_event(content)
...
def step_imp(context, content, source, source_type, host):
    ...
    assert context.containers["splunk"].check_splunk_event_with_attributes(content, attr)
