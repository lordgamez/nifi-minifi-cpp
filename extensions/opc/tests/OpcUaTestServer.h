...
class OpcUaTestServer {
 ...
 private:
  ...
  std::optional<std::vector<const HistoryModificationRecord*>> readNodeHistory(const UA_HistoryReadValueId& node_to_read,
      const UA_ReadRawModifiedDetails& details, UA_HistoryReadResult& result, bool& has_more_data) {
    ...
    if (it == history_records_.end() || node_id_it == node_ids_.end() ||
        node_to_read.nodeId.identifierType != node_id_it->second.identifierType) {
      result.statusCode = UA_STATUSCODE_BADNODEIDUNKNOWN;
      return std::nullopt;
    }
    return selectRecords(it->second, details.startTime, details.endTime, details.numValuesPerNode, has_more_data);
  }
  ...
  static void readRawCallback(UA_Server* /*server*/, void* hdb_context, const UA_NodeId* /*sessionId*/, void* /*session_context*/,
    ...
    for (size_t i = 0; i < nodes_to_read_size; ++i) {
      ...
      auto records = self->readNodeHistory(nodes_to_read[i], *history_read_details, response->results[i], has_more_data);
      if (!records) {
        continue;
      }
