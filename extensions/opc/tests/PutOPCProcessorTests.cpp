...
struct NodeData {
  ...
  // When set, the created node is expected to have a GUID identifier equal to this value; otherwise a numeric identifier equal to node_id is expected.
...
// Reads a node's value directly by its node id (no parent/path needed) and checks it equals expected_value.
...
TEST_CASE("Test creating a new node under a GUID parent node id", "[putopcprocessor]") {
  ...
  // The GUID object GuidObject is created under Simulator/Default/Device2 by the test server to act as a parent container.
