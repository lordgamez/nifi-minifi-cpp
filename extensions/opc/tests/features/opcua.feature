...
Feature: Putting and fetching data to OPC UA server
  ...
  Scenario: Update and fetch historical data from an OPC UA node in JSON format
    ...
    When all instances start up
    Then in the 'fetch-opc-ua-node-history' container at least one file in '/tmp/output' content match the following regex: '\[\{"Sourcetimestamp":".+","NamespaceIndex":"1","NodeID":"myUintValue","Value":"0"\},\{"Sourcetimestamp":".+","NamespaceIndex":"1","NodeID":"myUintValue","Value":"1"\},\{"Sourcetimestamp":".+","NamespaceIndex":"1","NodeID":"myUintValue","Value":"2"\}\]' in less than 30 seconds
