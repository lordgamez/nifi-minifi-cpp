...
TEST_CASE_METHOD(FetchOPCEventsTestController, "Events over the queue size limit are dropped and reported", "[fetchopcevents]") {
  ...
  REQUIRE(utils::verifyEventHappenedInPollTime(10s, [&]() {
    transferred += controller_.trigger().at(processors::FetchOPCEvents::Success).size();
    return LogTestController::getInstance().getLogs().find("The event queue is full") != std::string::npos;
  }, 100ms));
  CHECK(transferred < event_count);
}
