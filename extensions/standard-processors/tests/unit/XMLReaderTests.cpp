...
TEST_CASE("Specify Field Name for Content property for tagless values", "[XMLReader]") {
  ...
  XMLReader xml_reader("XMLReader");
  xml_reader.initialize();
  REQUIRE(xml_reader.setProperty(XMLReader::FieldNameForContent.name, "tagvalue"));
  xml_reader.onEnable();
  auto record_set = xml_reader.read(buffer_stream);
  REQUIRE(record_set);
  REQUIRE(record_set->size() == 1);
...
TEST_CASE("Parse attributes as record fields if Parse XML Attributes property is set", "[XMLReader]") {
  ...
  XMLReader xml_reader("XMLReader");
  xml_reader.initialize();
  REQUIRE(xml_reader.setProperty(XMLReader::ParseXMLAttributes.name, "true"));
  xml_reader.onEnable();
  auto record_set = xml_reader.read(buffer_stream);
  REQUIRE(record_set);
  REQUIRE(record_set->size() == 1);
...
TEST_CASE("Nodes clashing with the content field name are ignored", "[XMLReader]") {
...
TEST_CASE("Attributes are prefixed with the defined prefix", "[XMLReader]") {
  ...
  xml_reader.initialize();
  REQUIRE(xml_reader.setProperty(XMLReader::ParseXMLAttributes.name, "true"));
  REQUIRE(xml_reader.setProperty(XMLReader::FieldNameForContent.name, "fieldname"));
  REQUIRE(xml_reader.setProperty(XMLReader::AttributePrefix.name, "attr_"));
  xml_reader.onEnable();
  auto record_set = xml_reader.read(buffer_stream);
  REQUIRE(record_set);
  REQUIRE(record_set->size() == 1);
...
TEST_CASE("Read multiple records from XML", "[XMLReader]") {
  ...
  XMLReader xml_reader("XMLReader");
  xml_reader.initialize();
  REQUIRE(xml_reader.setProperty(XMLReader::ExpectRecordsAsArray.name, "true"));
  xml_reader.onEnable();
  auto record_set = xml_reader.read(buffer_stream);
  REQUIRE(record_set);
