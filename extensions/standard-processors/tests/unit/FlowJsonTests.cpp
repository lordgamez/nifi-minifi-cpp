      ...
      R"(
{
  ...
  auto* port = dynamic_cast<minifi::RemoteProcessGroupPort*>(flow->findProcessorByName("AmazingInputPort"));
  REQUIRE(port);
      ...
      R"(
{
  ...
  auto* port = dynamic_cast<minifi::RemoteProcessGroupPort*>(flow->findProcessorByName("AmazingOutputPort"));
  REQUIRE(port);
  CHECK(port->getUUIDStr() == "00000000-0000-0000-0000-000000000003");
  CHECK(port->getUseCompression() == true);
  CHECK(port->getBatchSize().value() == 10);
  CHECK(port->getBatchCount().value() == 3);
  CHECK(port->getBatchDuration().value() == std::chrono::seconds(5));
  CHECK(port->getTimeout().value() == std::chrono::seconds(19));
}
