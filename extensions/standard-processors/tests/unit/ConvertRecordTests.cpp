...
TEST_CASE("Record conversion fails with read failure", "[ConvertRecord]") {
  ...
  CHECK(*output_flow_file->getAttribute(processors::ConvertRecord::RecordErrorMessageOutputAttribute.name) == "Invalid argument");
  CHECK(controller.plan->getContent(output_flow_file) == "<invalidxml>");
  CHECK(LogTestController::getInstance().contains("Failed to read record set from flow file: Invalid argument"));
}
