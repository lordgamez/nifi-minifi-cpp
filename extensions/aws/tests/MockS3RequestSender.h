...
class MockS3RequestSender : public minifi::aws::s3::S3RequestSender {
 public:
  ...
  std::optional<Aws::S3Crt::Model::UploadPartResult> sendUploadPartRequest(const Aws::S3Crt::Model::UploadPartRequest& request) override {
    ...
    // Consume the body like the real SDK, so the underlying io::InputStream
    // advances to the end of this part — allowing the next part to start
    // at the correct position without any explicit seek in production code.
