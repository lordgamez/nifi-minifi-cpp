...
std::optional<PutObjectResult> S3Wrapper::putObject(const PutObjectRequestParameters& put_object_params, const std::shared_ptr<io::InputStream>& stream, uint64_t flow_size) {
  ...
  // Stream directly from io::InputStream to avoid buffering the entire file in memory.
  // SetContentLength is called explicitly so the AWS SDK never seeks the stream to determine
  // its size — on Windows, MSVC's std::stringbuf::seekoff uses a 32-bit int internally
  // which overflows for streams larger than 2 GB, putting the stream in a bad state.
  auto aws_stream = std::make_shared<MinifiToAwsInputStream>(stream, flow_size);
  request.SetBody(aws_stream);
  request.SetContentLength(static_cast<long long>(flow_size));
...
std::optional<S3Wrapper::UploadPartsResult> S3Wrapper::uploadParts(const PutObjectRequestParameters& put_object_params, const std::shared_ptr<io::InputStream>& stream,
  ...
  for (size_t part_number = start_part; part_number <= last_part; ++part_number) {
    ...
    // Stream each part directly from io::InputStream without buffering in memory.
    // SetContentLength is set explicitly so the SDK never seeks the stream to determine
    // its size, avoiding the MSVC std::stringbuf::seekoff 32-bit int overflow for parts > 2 GB.
    ...
    upload_part_request.SetContentLength(static_cast<long long>(next_read_size));
...
void S3Wrapper::initializeMultipartUploadStateStorage(core::StateManager& state_manager) {
