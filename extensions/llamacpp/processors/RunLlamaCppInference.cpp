...
void RunLlamaCppInference::onTrigger(core::ProcessContext& context, core::ProcessSession& session) {
  ...
  result->setAttribute(LlamaCppTimeToFirstToken.name, std::to_string(generation_result->time_to_first_token.count()) + " ms");
  result->setAttribute(LlamaCppTokensPerSecond.name, std::to_string(generation_result->tokens_per_second));
