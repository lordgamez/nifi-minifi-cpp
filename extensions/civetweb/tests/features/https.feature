...
Feature: Transfer data from and to MiNiFi using HTTPS
  ...
  Scenario: InvokeHTTP to ListenHTTP without an SSLContextService works (no mutual TLS in this case)
    ...
    And a InvokeHTTP processor with the "Remote URL" property set to "https://server-${scenario_id}:4430/contentListener"
    ...
    And in the "server" flow the "success" relationship of the ListenHTTP processor is connected to the PutFile
    And PutFile's success relationship is auto-terminated in the "server" flow

    When both instances start up
    Then in the "server" container at least one file with the content "Lorem ipsum dolor sit amet" is placed in the "/tmp/output" directory in less than 10 seconds
  ...
  Scenario: InvokeHTTP to ListenHTTP without an SSLContextService requires a server cert signed by a CA
    ...
    When both instances start up
    Then in the "server" container no files are placed in the "/tmp/output" directory in 10s of running time
  ...
  Scenario: InvokeHTTP to ListenHTTP with mutual TLS, using certificate files
    ...
    And a ListenHTTP processor with the "Listening Port" property set to "4430" in the "server" flow
    And the "SSL Certificate" property of the ListenHTTP processor is set to "/tmp/resources/minifi_server.crt" in the "server" flow
    And the "SSL Certificate Authority" property of the ListenHTTP processor is set to "/usr/local/share/certs/ca-root-nss.crt" in the "server" flow
    And the "SSL Verify Peer" property of the ListenHTTP processor is set to "yes" in the "server" flow
    And a PutFile processor with the "Directory" property set to "/tmp/output" in the "server" flow
    And PutFile is EVENT_DRIVEN in the "server" flow
    And in the "server" flow the "success" relationship of the ListenHTTP processor is connected to the PutFile
    And PutFile's success relationship is auto-terminated in the "server" flow
    ...
    When both instances start up
    Then in the "server" container at least one file with the content "ut labore et dolore magna aliqua" is placed in the "/tmp/output" directory in less than 10s
  ...
  Scenario: InvokeHTTP to ListenHTTP without mutual TLS, using the system certificate store
    ...
    When both instances start up
    Then in the "server" container at least one file with the content "Ut enim ad minim veniam" is placed in the "/tmp/output" directory in less than 10s
  ...
  Scenario: InvokeHTTP to ListenHTTP with mutual TLS, using the system certificate store
    ...
    When both instances start up
    Then in the "server" container at least one file with the content "quis nostrud exercitation ullamco laboris nisi" is placed in the "/tmp/output" directory in less than 10s
