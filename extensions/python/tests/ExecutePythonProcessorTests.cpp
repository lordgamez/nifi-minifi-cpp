...
class SimplePythonFlowFileTransferTest : public ExecutePythonProcessorTestBase {
 ...
 protected:
  ...
  void testsStatefulProcessor() {
    ...
    plan_->addProcessor("ExecutePythonProcessor", "executePythonProcessor", core::Relationship("success", "description"), false, [this](auto& processor) {
      auto execute_python_processor = dynamic_cast<minifi::extensions::python::processors::ExecutePythonProcessor*>(&processor);
  ...
  core::Processor* addExecutePythonProcessorToPlan(const std::filesystem::path& used_as_script_file) {
    auto processor = plan_->addProcessor("ExecutePythonProcessor", "executePythonProcessor", core::Relationship("success", "description"), true, [&](auto& processor) {
      auto execute_python_processor = dynamic_cast<minifi::extensions::python::processors::ExecutePythonProcessor*>(&processor);
