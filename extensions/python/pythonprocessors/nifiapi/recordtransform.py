    ...
    def onTrigger(self, context: ProcessContext, session: ProcessSession):
        ...
        for result in results:
            ...
            if record_partition in partitions:
                partitions[record_partition].add(result)
            else:
                partitions[record_partition] = [result]

        for _, partition_results in partitions.items():
            partitioned_flow_file = session.create(flow_file)
            record_writer.write([result.getRecordJson() for result in partition_results], partitioned_flow_file, session)
            ...
            session.transfer(partitioned_flow_file, result.getRelationship())
