...
@then(
    'in the "{container_name}" container a single file with the content "{content}" is placed in the "{directory}" directory in less than {duration}'
)
def verify_single_file_content_in_container_directory(
    ...
    timeout_in_seconds = humanfriendly.parse_timespan(duration)
    assert wait_for_condition(
        condition=lambda: context.containers[container_name].directory_has_single_file_with_content(
...
def verify_single_file_content_in_directory(context: MinifiTestContext, content: str, directory: str, duration: str):
    context.execute_steps(
        f'then in the "{DEFAULT_MINIFI_CONTAINER_NAME}" container a single file with the content "{content}" is placed in the "{directory}" directory in less than {duration}'
    )
...
@then("at least one file with the content '{content}' is placed in the '{directory}' directory in less than {duration}")
def verify_at_least_one_file_content_in_directory(
    context: MinifiTestContext, content: str, directory: str, duration: str
):
    context.execute_steps(
        f'then in the "{DEFAULT_MINIFI_CONTAINER_NAME}" container at least one file with the content "{content}" is placed in the "{directory}" directory in less than {duration}'
    )


@then("the logs of the '{container}' container do not contain the following message: '{message}' after {duration}")
@then('the logs of the "{container}" container do not contain the following message: "{message}" after {duration}')
def verify_container_logs_do_not_contain_message(
    context: MinifiTestContext, container: str, message: str, duration: str
):
...
def verify_minifi_logs_do_not_contain_message(context: MinifiTestContext, message: str, duration: str):
    context.execute_steps(
        f'then the logs of the "{DEFAULT_MINIFI_CONTAINER_NAME}" container do not contain the following message: "{message}" after {duration}'
    )
...
def verify_minifi_logs_do_not_contain_warnings(context: MinifiTestContext):
    assert "[warning]" not in context.containers[DEFAULT_MINIFI_CONTAINER_NAME].get_logs() or log_due_to_failure(
        context
    )
...
def verify_container_logs_contain_message(context: MinifiTestContext, container: str, message: str, duration: str):
    duration_seconds = humanfriendly.parse_timespan(duration)
    assert wait_for_condition(
        condition=lambda: message in context.containers[container].get_logs(),
        timeout_seconds=duration_seconds,
        bail_condition=lambda: context.containers[container].exited,
        context=context,
    )
...
def verify_minifi_logs_contain_message(context: MinifiTestContext, message: str, duration: str):
    context.execute_steps(
        f'then the logs of the "{DEFAULT_MINIFI_CONTAINER_NAME}" container contain the following message: "{message}" in less than {duration}'
    )
...
def verify_minifi_logs_contain_message_multiple_times(context, log_message, count, duration):
    duration_seconds = humanfriendly.parse_timespan(duration)
    time.sleep(duration_seconds)
    assert context.containers[DEFAULT_MINIFI_CONTAINER_NAME].get_logs().count(
        log_message
...
def verify_minifi_logs_match_regex(context, regex, duration):
    duration_seconds = humanfriendly.parse_timespan(duration)
    assert wait_for_condition(
        condition=lambda: re.search(regex, context.containers[DEFAULT_MINIFI_CONTAINER_NAME].get_logs()),
        timeout_seconds=duration_seconds,
        bail_condition=lambda: context.containers[DEFAULT_MINIFI_CONTAINER_NAME].exited,
        context=context,
    )
...
def verify_no_errors_on_http_proxy(context: MinifiTestContext, url: str):
    http_proxy_container = next(
        container for container in context.containers.values() if isinstance(container, HttpProxy)
    )
...
def verify_no_files_in_container_directory(context, container, directory, duration):
    duration_seconds = humanfriendly.parse_timespan(duration)
    assert check_condition_after_wait(
        condition=lambda: context.containers[container].get_number_of_files(directory) == 0,
        context=context,
...
def verify_no_files_in_directory(context, directory, duration):
    context.execute_steps(
        f'then in the "{DEFAULT_MINIFI_CONTAINER_NAME}" container no files are placed in the "{directory}" directory in {duration} of running time'
    )
...
def verify_number_of_files_in_directory(context: MinifiTestContext, num: int, directory: str, duration: str):
    ...
    if num == 0:
        ...
        return
    assert wait_for_condition(
        condition=lambda: context.containers[DEFAULT_MINIFI_CONTAINER_NAME].get_number_of_files(directory) == num,
        timeout_seconds=duration_seconds,
        bail_condition=lambda: context.containers[DEFAULT_MINIFI_CONTAINER_NAME].exited,
        context=context,
    )
...
@then('in the "{container_name}" container at least one file in "{directory}" content match the following regex: "{regex_str}" in less than {duration}')
@then("in the '{container_name}' container at least one file in '{directory}' content match the following regex: '{regex_str}' in less than {duration}")
@then("in the '{container_name}' container the content of at least one file in the \"{directory}\" directory matches the '{regex_str}' regex in less than {duration}"
)
def verify_file_content_matches_regex_in_directory_in_container(
    ...
    duration_seconds = humanfriendly.parse_timespan(duration)
    assert wait_for_condition(
        condition=lambda: context.containers[container_name].directory_contains_file_with_regex(
            directory, regex_str
        ),
        timeout_seconds=duration_seconds,
        bail_condition=lambda: context.containers[container_name].exited,
...
def verify_file_content_matches_regex_in_directory(context: MinifiTestContext, directory: str, regex_str: str, duration: str):
    context.execute_steps(f'then in the "{DEFAULT_MINIFI_CONTAINER_NAME}" container at least one file in "{directory}" content match the following regex: "{regex_str}" in less than {duration}')


@then(
    'files with contents "{content_one}" and "{content_two}" are placed in the "{directory}" directory in less than {timeout}'
)
@then(
    "files with contents '{content_one}' and '{content_two}' are placed in the '{directory}' directory in less than {timeout}"
)
def verify_files_with_two_contents_in_directory(
    context: MinifiTestContext,
    directory: str,
    timeout: str,
    content_one: str,
    ...
    contents_arr = [c1, c2]
    assert wait_for_condition(
        condition=lambda: context.containers[DEFAULT_MINIFI_CONTAINER_NAME].verify_file_contents(
            directory, contents_arr
        ),
...
@then('in the "{container_name}" container files with at least these contents "{contents}" are placed in the "{directory}" directory in less than {timeout}')
def verify_files_with_at_least_contents_in_directory_in_container(context: MinifiTestContext, container_name: str, directory: str, timeout: str, contents: str):
    timeout_seconds = humanfriendly.parse_timespan(timeout)
    new_contents = contents.replace("\\n", "\n")
    contents_arr = new_contents.split(",")
    assert wait_for_condition(
...
def verify_files_with_at_least_contents_in_directory(context: MinifiTestContext, directory: str, timeout: str, contents: str):
    context.execute_steps(f'then in the "{DEFAULT_MINIFI_CONTAINER_NAME}" container files with at least these contents "{contents}" are placed in the "{directory}" directory in less than {timeout}')


@then('in the "{container_name}" container a file with the JSON content "{content}" is placed in the "{directory}" directory in less than {duration}')
@then("in the '{container_name}' container a file with the JSON content '{content}' is placed in the '{directory}' directory in less than {duration}")
def verify_file_with_json_content_in_directory_in_container(context: MinifiTestContext, container_name: str, content: str, directory: str, duration: str):
    timeout_in_seconds = humanfriendly.parse_timespan(duration)
    assert wait_for_condition(
        condition=lambda: context.containers[container_name].verify_path_with_json_content(
            directory, content
        ),
        timeout_seconds=timeout_in_seconds,
        bail_condition=lambda: context.containers[container_name].exited,
...
def verify_file_with_json_content_in_directory(context: MinifiTestContext, content: str, directory: str, duration: str):
    context.execute_steps(f'then in the "{DEFAULT_MINIFI_CONTAINER_NAME}" container a file with the JSON content "{content}" is placed in the "{directory}" directory in less than {duration}')
...
@then(
    'at least one file with the JSON content "{content}" is placed in the "{directory}" directory in less than {duration}'
)
@then(
    "at least one file with the JSON content '{content}' is placed in the '{directory}' directory in less than {duration}"
)
def verify_at_least_one_file_with_json_content_in_directory(
    context: MinifiTestContext, content: str, directory: str, duration: str
):
    timeout_in_seconds = humanfriendly.parse_timespan(duration)
    assert wait_for_condition(
        condition=lambda: context.containers[DEFAULT_MINIFI_CONTAINER_NAME].directory_contains_file_with_json_content(
            directory, content
        ),
        timeout_seconds=timeout_in_seconds,
        bail_condition=lambda: context.containers[DEFAULT_MINIFI_CONTAINER_NAME].exited,
...
@then(
    'after a wait of {duration}, at least {lower_bound:d} and at most {upper_bound:d} files are produced and placed in the "{directory}" directory'
)
def verify_file_count_bounds_in_directory(
    context: MinifiTestContext,
    lower_bound: int,
    upper_bound: int,
    duration: str,
    ...
    assert check_condition_after_wait(
        condition=lambda: (
            context.containers[DEFAULT_MINIFI_CONTAINER_NAME].get_number_of_files(directory) >= lower_bound
...
@then(
    'in the "{container_name}" container at least one empty file is placed in the "{directory}" directory in less than {duration}'
)
def verify_at_least_one_empty_file_in_container_directory(
    context: MinifiTestContext, container_name: str, directory: str, duration: str
):
    timeout_in_seconds = humanfriendly.parse_timespan(duration)
    assert wait_for_condition(
...
@then(
    'in the "{container_name}" container at least one file with minimum size of "{size}" is placed in the "{directory}" directory in less than {duration}'
)
def verify_file_with_minimum_size_in_container_directory(
    context: MinifiTestContext,
    container_name: str,
    directory: str,
    size: str,
    ...
    assert wait_for_condition(
        condition=lambda: context.containers[container_name].directory_contains_file_with_minimum_size(
...
@then(
    'at least one file with minimum size of "{size}" is placed in the "{directory}" directory in less than {duration}'
)
