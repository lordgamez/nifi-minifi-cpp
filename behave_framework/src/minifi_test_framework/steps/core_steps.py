...
def step_impl(context: MinifiTestContext, content: str, path: str):
    new_content = content.replace("\\n", "\n")
    context.get_or_create_default_minifi_container().files.append(File(path, str(uuid.uuid4()), new_content))


@given("an empty file is present in \"{path}\"")
def step_impl(context, path):
    context.get_or_create_default_minifi_container().files.append(File(path, str(uuid.uuid4()), ""))
