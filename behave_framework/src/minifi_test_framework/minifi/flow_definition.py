...
class FlowDefinition(ABC):
    ...
    def add_input_port_to_rpg(self, rpg_name: str, port_name: str):
        rpg = next((rpg for rpg in self.remote_process_groups if rpg.name == rpg_name), None)
        if rpg:
            rpg.add_input_port(port_name)
        else:
            raise ValueError(f"RemoteProcessGroup with name '{rpg_name}' not found.")

    def add_output_port_to_rpg(self, rpg_name: str, port_name: str):
        rpg = next((rpg for rpg in self.remote_process_groups if rpg.name == rpg_name), None)
        if rpg:
            rpg.add_output_port(port_name)
        else:
            raise ValueError(f"RemoteProcessGroup with name '{rpg_name}' not found.")
