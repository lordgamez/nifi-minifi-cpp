...
#ifdef WIN32
  static constexpr std::string_view OPENSSL_BINARY = "openssl.exe";
#else
  static constexpr std::string_view OPENSSL_BINARY = "openssl";
#endif
  std::string command = (minifi_home / "fips" / OPENSSL_BINARY).string() + " fipsinstall -out " + output_file.string() + " -module " + (minifi_home / "fips" / FIPS_LIB).string();
