    ...
    */
    ...
    GenericValue& operator[](const GenericValue<Encoding, SourceAllocator>& name) {
        ...
        if (member != MemberEnd())
            ...
            return *new (buffer) GenericValue();  // NOLINT
        }
    }
    template <typename SourceAllocator>
...
public:
    ...
    // the next line is patched by szaszm, in nifi-minifi-cpp, to avoid ambiguity with the implicit constructor on line 872
    template <typename U = ValueType> operator typename std::enable_if<!std::is_convertible<GenericObject&&, U>::value, U&>::type() const { return value_; }
