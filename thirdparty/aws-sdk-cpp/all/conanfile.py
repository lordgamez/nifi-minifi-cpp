...
class AwsSdkCppConan(ConanFile):
    ...
    def requirements(self):
        # The aws-crt-cpp / aws-c-* / aws-checksums / s2n dependencies are built in-tree from
        # the SDK's own git submodules (BUILD_DEPS=ON), so they are not required as conan
        # packages here - this guarantees the exact CRT versions this SDK tag expects.
        if self.settings.os != "Windows":
    ...
    def source(self):
        # The release tarball does not contain the CRT submodules, so we clone the repo and
        # initialize the submodules recursively - this pins the exact, mutually-compatible CRT
        # versions for this SDK tag (mirrors cmake/BundledAwsSdkCpp.cmake).
        # init + fetch (rather than clone) so this coexists with the exported patches/ folder
        # that conan places in the source folder.
        git = Git(self)
        git.run("init .")
        ...
        # Apply the patches from conandata. The bundled OpenSSL-path patches (s2n / aws-c-cal)
        # are intentionally not applied here: with conan providing OpenSSL, the SDK's stock
        # find_package(OpenSSL) / find_package(crypto) (via the in-tree Findcrypto.cmake) resolve
        # against the conan package instead (see OPENSSL_ROOT_DIR in generate()).
        for patch_data in self.conan_data.get("patches", {}).get(self.version, []):
            patch_file = patch_data["patch_file"]
            # strip=0: the patches are git-format, so patch_ng already drops the a/ b/ prefix
            # (equivalent to `patch -p1` used by cmake/BundledAwsSdkCpp.cmake).
    ...
    def generate(self):
        ...
        # Build the CRT dependencies from the in-tree submodules instead of expecting them
    ...
    def package_info(self):
        ...
        core.requires = ["aws-crt-cpp", "aws-c-event-stream", "zlib::zlib"]
        if not is_windows:
            core.requires.extend(["libcurl::curl", "openssl::openssl"])
