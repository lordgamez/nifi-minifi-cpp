...
class GoogleCloudCppConan(ConanFile):
    ...
    def package(self):
        ...
        if self.options.with_mocks:
            # Upstream only installs testing/mock_client.h. The header-only
            # storage testing helpers used by consumers also include
            # canonical_errors.h, so ship it alongside the mocks.
