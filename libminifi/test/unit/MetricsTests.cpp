...
class DuplicateContentProcessor : public minifi::core::Processor {
 ...
 public:
  ...
  void onTrigger(core::ProcessContext& /*context*/, core::ProcessSession& session) override {
    ...
    session.read(flow_file, [&, this](const std::shared_ptr<io::InputStream>& stream) -> int64_t {
      buffer.resize(stream->size());
      return stream->read(buffer);
    });
    session.write(flow_file_copy, [&, this](const std::shared_ptr<io::OutputStream>& stream) {
      return stream->write(buffer);
    });
    session.append(flow_file_copy, [&, this](const std::shared_ptr<io::OutputStream>& stream) {
      return stream->write(buffer);
    });
