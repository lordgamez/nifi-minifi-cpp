...
class MetricsHandler: public HeartbeatHandler {
 ...
 private:
  ...
  static constexpr const char* LOGATTRIBUTE_UUID = "2438e3c8-015a-1000-79ca-83af40ec1992";
  ...
  static bool verifyRuntimeMetrics(const rapidjson::Value& runtime_metrics) {
    return runtime_metrics.HasMember("deviceInfo") &&
      ...
      [&]() {
        ...
        for (const auto& processor : runtime_metrics["flowInfo"]["processorStatuses"].GetArray()) {
          if (processor["id"].GetString() != std::string(GETTCP1_UUID) && processor["id"].GetString() != std::string(LOGATTRIBUTE_UUID)) {
