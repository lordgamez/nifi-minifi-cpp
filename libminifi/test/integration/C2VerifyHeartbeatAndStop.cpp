...
class VerifyC2Heartbeat : public VerifyC2Base {
 public:
  ...
  void runAssertions() override {
    REQUIRE(utils::verifyLogLinePresenceInPollTime(std::chrono::milliseconds(wait_time_),
    ...
    CHECK(utils::countLogOccurrencesUntil("C2Agent] [debug] Stopping all processors", 2, 300ms, 50ms));
    CHECK(utils::countLogOccurrencesUntil("C2Agent] [debug] Starting all processors", 2, 300ms, 50ms));
  }
