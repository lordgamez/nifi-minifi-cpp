...
TEST_CASE("The JSON schema detects invalid values in the json flow") {
  ...
  ErrorHandler err_handler{[&] (const auto& err) {
    ...
    if (it == errors.end()) {
      // json-schema-validator v2.4.0 reports two errors per allOf failure:
      // 1. "at least one subschema has failed... <msg>" and 2. "[combination: allOf / case#N] <msg>"
      // Ignore the second duplicate error for paths already consumed.
