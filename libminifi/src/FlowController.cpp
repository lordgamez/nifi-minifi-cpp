...
nonstd::expected<void, std::string> FlowController::applyConfiguration(const std::string &source, const std::string &configurePayload, const std::optional<std::string>& flow_id) {
  ...
  try {
    // Only parse the new flow here; do NOT call clearControllerServices() yet.
    // Clearing the CSP before stop() would cause a race with the onScheduleTimer_
    // callback thread (which may be calling getOrCreateDefaultStateStorage()).
