...
std::shared_ptr<core::StateStorage> StateManagementWrapper::getOrCreateDefaultStateStorage(
...
StateManager* StateManagementWrapper::getStateManager(const CoreComponent& component) {
  std::lock_guard<std::mutex> lock(state_manager_mutex_);
  if (state_storage_ == nullptr) { return nullptr; }
  if (!state_manager_) { state_manager_ = state_storage_->getStateManager(component); }
