...
# A conan (CMakeDeps) package exposes each library through an INTERFACE target whose actual
# IMPORTED archive is referenced in INTERFACE_LINK_LIBRARIES as a CONAN_LIB::* target - the
# INTERFACE target itself has no TARGET_FILE. Everything else (real libraries, or targets
# defined later in the same scope) is returned as-is and resolved by $<TARGET_FILE:...> at
# generate time. Only used on Apple/Windows, where whole-archiving needs an explicit file.
...
function(target_wholearchive_library_private TARGET ITEM)
    ...
    # add_dependencies is needed to order the bundled AWS ExternalProject build; conan
    # packages are already built, and their INTERFACE targets cannot be a dependency.
    if(TARGET ${ITEM})
        get_target_property(_item_type ${ITEM} TYPE)
